...
- **Facilitator-run resources (23.2)**: three free-standing, uncosted facilitator-only actions, each independent of the others and of a draw — nothing links a stone the facilitator adds to the pool, or a session context they create, with the draw that just happened. `POST /stones/add` / `/stones/remove` (`{ kind: "Boon" | "Bane" }`) hand-edit the pool directly — `add` always succeeds, `remove` 400s if the pool holds none of that kind, and neither posts a log line (silent bookkeeping, like the facilitator's own `add-boon`). `POST /stones/floating-boons` (`{ text }`) and `POST /stones/floating-boons/:id/delete` create and remove a session context directly, the same `FloatingBoon` shape an accepted Add a Detail / Gain Insight produces, without routing through that proposal — both post a log line (`Session note added/removed — <text>`), since a context note is worth recording. Adjusting a player's boons needed no new route: `/characters/:slot/fate` (section 4) already covers it, surfaced client-side as the facilitator's Grant `+` / `−` next to each sheet. `FloatingBoon` stays Boon-only until 23.3 widens it with a `kind`.
