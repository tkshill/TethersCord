...
- **`View/*.elm`** — one module per section card: `TopBar` (the persistent bar above the panels — the running goal and the Overcome "stage" (26.3): the pool as stones, Overcome for any player, the pending draw lifted beside it, and the facilitator's Reroll / Reject / Accept; session start/end/goal-edit sit behind a facilitator-only expander), `FacilitatorPanel` (the left panel's facilitator-only section — direct pool edits and the proposal queue, whose header counts what is waiting), `SessionAspects` (the centre column's "Session boons & banes" card — the session boons and banes list, renamed and moved here from the old Stones card by 23.5, as 23.3 anticipated), `Session` (now just the past-sessions history list, its goal/controls having moved to `TopBar`), `Moves` (real buttons for the five player moves, each disabled when unaffordable), `Characters`, `Entities`, `Log`, and `Guide` (the collapsible "How to play" glossary, which reads `Copy.Terms`). Each exposes `view : ViewContext -> <props record> -> GameState -> Element Msg` (a couple take no extra props; `Guide` takes no `GameState`). `View/Helpers.elm` holds the `ViewContext` type and the cross-section helpers (`placeholder`, `inputAttrs`, `characterLabel`, `countProposals` / `latestProposalId`, `pendingHint` / `withdrawLink`, `stoneChip`, and the glossary-tooltip helpers `glossaryTitle` / `tip` / `tipAttrs` over `Ui.withTip` — `tip`/`withTip` shrink-wrap, so a full-width element takes `tipAttrs` directly instead, to leave its width alone).
