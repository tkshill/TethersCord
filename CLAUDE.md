...
- **Facilitator-run resources (23.2)**: three free-standing, uncosted facilitator-only actions, each independent of the others and of a draw — nothing links a stone the facilitator adds to the pool, or a session context they create, with the draw that just happened. `POST /stones/add` / `/stones/remove` (`{ kind: "Boon" | "Bane" }`) hand-edit the pool directly — `add` always succeeds, `remove` 400s if the pool holds none of that kind, and neither posts a log line (silent bookkeeping, like the facilitator's own `add-boon`); the client surfaces these as a Boon `+`/`−` pair and a Bane `+`/`−` pair (`View/Stones.elm`'s `poolControls`, facilitator-only). `POST /stones/floating-boons` (`{ kind, text }`, widened 23.3) and `POST /stones/floating-boons/:id/delete` create and remove a session context directly — the facilitator's choice of `kind`, unlike an accepted Add a Detail / Gain Insight, which always produces a Boon — without routing through that proposal; both post a log line (`Session note added/removed (<kind>) — <text>`), since a context note is worth recording. The client's `floatingBoonsBlock` shows each one as a kind-coloured chip (`floatingBoonChip`), a facilitator-only "Remove" per row, and an always-visible add row with a Boon/Bane picker and text field. Adjusting a player's boons needed no new route: `/characters/:slot/fate` (section 4) already covers it, surfaced client-side as the facilitator's Grant `+` / `−` next to each sheet — now the *only* client path onto a character's boons, since the player-facing "Add boon" button is also removed (facilitator adds a Boon directly instead) alongside Pledge.
