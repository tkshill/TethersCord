...
- **`RULES.md` is the source of truth for what the rules are** (the Overcome loop, the five moves and their costs, session boons and banes); this section only records how the Worker implements them. The client has not been brought up to the 26.2 wire shape yet (roadmap 26.3), so `main` is not deployable until 26.3 lands.
...
- **`Kind.elm`** — the `ProposalKind` / `AbilityKind` custom types (with decoders and string encoders) at parity with the Worker's `types.ts` unions. Its own module because `Complicate` / `AddBoon` already name `Msg` variants.
