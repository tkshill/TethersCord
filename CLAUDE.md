...
- **One shared stone pool** (`gameState.stonePool`) backs every roll — overcomes, plain rolls, and the session-end top-up all read and write the same pool; there is no separate per-session pool anymore, and nothing ever resets it wholesale. A cold table starts it at the base four (`INITIAL_STONE_POOL`: two Boon, two Bane); from there it only changes through rolls and moves.
- **The overcome**: `gameState.overcome` (`{ targetSlot } | null`, in `KEY_STONES` storage) is a facilitator-framed risky attempt against one character. `/overcome/start` (`{ slot }`) and `/overcome/cancel` are facilitator-only; while an overcome is open, `/stones/roll` and `/stones/reroll` are also allowed to the player who owns the target sheet (`rollGate`, not `facilitatorOnly`), and a Reroll deducts `REROLL_COST` (2) boons from the target's `fate` (the "Press Fate" move). Accepting the roll (`/stones/accept`) removes the two drawn stones from the pool, then **routes** them back (`routeOvercomeDraw` in `gameLogic.ts`): two of a kind both return to the pool whole; a mixed roll returns only the Boon and drops the Bane onto one of the target's three aspects at random (`archetype_banes` / `desire_banes` / `quest_banes` on `characters`, migration `0009`), surfaced on the sheet. A plain (non-overcome) roll returns one random result stone the same way — whatever isn't routed back is simply gone, since there is no reset to fall back on.
...
- **The session goal is plain text with no verdict**: `SessionState` is just `{ id, goal }`. Ending a session (`/session/end`) does not roll anything and does not judge the goal — it only tops the shared pool back up to at least two Boon and two Bane if either has fallen short, so scarcity from overcomes never compounds past a playable floor. `game_sessions.outcome` is no longer written; session history (`gameState.sessionHistory`) shows only each past session's goal and dates. Untethering, the reckoning mechanic that used to follow a failed goal, has been retired along with the goal verdict that triggered it — aspect Banes (`archetype_banes` / `desire_banes` / `quest_banes`) still accumulate from mixed overcome rolls, but nothing currently clears them.
