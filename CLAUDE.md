...
- **Session verdict, carry, and untethering (section 19)**: `/session/end` draws **one** stone from the session pool — a Boon means the goal is met, a Bane means it failed. On success the pool's Boons flush and its Banes carry into the next session (`session.carriedBanes`, seeded from the `carriedBanes` field of `KEY_STONES`); a failure flushes the carry to zero. A failed goal also **untethers** one character: one Bane is drawn at random from every aspect Bane on every sheet, and that character is untethered on the drawn aspect (`gameState.untether`, `{ slot, aspect } | null`) with all three of their aspect-Bane columns cleared. Skipped if a reckoning is already open, if the previous session also failed (`lastSessionFailed`), or if nobody carries an aspect Bane. `POST /untether/resolve` (`facilitatorOnly`) closes it. While untethered a character's own mixed overcome Bane goes to the pool instead of an aspect (the "frenzy").
...
- **`Copy.elm`** / **`Copy/Terms.elm`** — `Copy.elm` is every player-facing string the view renders, as named constants grouped by card (small formatting functions where a value is spliced in); structural labels that are not game vocabulary ("Name", "Notes") stay inline. `Copy/Terms.elm` is the glossary: `Term = { term, short, long }` and `terms` / `groupedTerms`, the `short` feeding the `title` tooltips and the `long` the `View.Guide` card.
