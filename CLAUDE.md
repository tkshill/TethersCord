...
- Mutations — `POST /message`, `/messages/clear`, `/stones/{add-boon,commit,draw,use-floating}`, `/abilities/use`, `/moves/accept-compel`, `/proposals/:id/{accept,reject,withdraw}`, `/session/{start,end,goal}`, `/characters/:slot/{update,fate,claim,release}`, `/{npcs,locations}` and `/{npcs,locations}/:id/{update,delete}` — return **`204` only**. They do not return a snapshot, on purpose: an HTTP body would race the WebSocket broadcast that already went out, and the two transports have no ordering. `/stones/commit`, `/abilities/use`, `/moves/accept-compel`, `/stones/use-floating` and the `claim`/`release` routes resolve the character sheet from the caller's Discord id (`characters.discord_user_id`), not a slot argument.
- **One shared stone pool** (`gameState.stonePool`) backs every draw — a draw (`/stones/draw`) only ever *reads* it, so the facilitator's direct pool edits and the session-end top-up are the only writes; there is no separate per-session pool, and nothing ever resets it wholesale. A cold table starts it at the base four (`INITIAL_STONE_POOL`: two Boon, two Bane).
